...
function showJumpDialog(system, target, distance)
  ...
  if fuelAmount then
    ...
    if player.isAdmin() or fuelAmount >= cost then
	  ...
	  widget.setText("jumpDialog.dtext", string.format(config.getParameter("jumpDialogDistance.distance"), util.round(distance)))
...
function showJumpDialogInSystem(travel)
	local fuelAmount = world.getProperty("ship.fuel")
	local cost = fuelCostInSystem()
	if fuelAmount then
		widget.setVisible("jumpDialogInSystem", true)

		if player.isAdmin() or fuelAmount >= cost then
			widget.setText("jumpDialogInSystem.text", string.format(config.getParameter("jumpDialogInSystem.valid"), cost))
			widget.setButtonEnabled("jumpDialogInSystem.jump", true)
			widget.setData("jumpDialogInSystem.jump", {travel = travel})

			self.travel.confirmed = false
		else
			widget.setText("jumpDialogInSystem.text", string.format(config.getParameter("jumpDialogInSystem.invalid"), cost))
			widget.setButtonEnabled("jumpDialogInSystem.jump", false)
			widget.setData("jumpDialogInSystem.jump", nil)
		end
	end
end

function hideJumpDialogInSystem()
	if widget.active("jumpDialogInSystem") then
		widget.setData("jumpDialogInSystem.jump", nil)
		widget.setVisible("jumpDialogInSystem", false)
...
function confirmJumpInSystem(_, jump)
	local fuelAmount = world.getProperty("ship.fuel")
	local cost = fuelCostInSystem()
	if jump and fuelAmount and fuelAmount >= cost then
		if jump.travel then
			self.travel = jump.travel
			self.travel.confirmed = true
			world.setProperty("ship.fuel", world.getProperty("ship.fuel") - cost)
		end
	end
	hideJumpDialogInSystem()
end

function cancelJumpInSystem()
	hideJumpDialogInSystem()
end
...
function fuelCost(travel)
  ...
  self.origin = celestial.currentSystem()
  self.destination = { location = travel or self.travel.system, planet = 0, satellite = 0, system = self.travel.target }
  
  local distanceCalc = math.sqrt( ( (self.origin.location[1] - self.destination.location[1]) ^ 2 ) + ( (self.origin.location[2] - self.destination.location[2]) ^ 2 ) )
  local shipLevel = player.shipUpgrades().shipLevel
  
  local ftlJumpCost = math.max(config.getParameter("jumpFuelCost"), config.getParameter("jumpFuelCost") * (shipLevel / 2))
  
  cost = ( distanceCalc * shipLevel/2 + ( distanceCalc^2 / ( 100 + 5 * shipLevel ))) + ftlJumpCost
  
  return util.round(cost - cost * (world.getProperty("ship.fuelEfficiency") or 0.0))
end

function fuelCostInSystem()
	local cost = config.getParameter("jumpFuelCostInSystem")
	return util.round(cost * player.shipUpgrades().shipLevel)
end
  ...
  while true do
    ...
    if self.travel.system then
      ...
      elseif self.travel.confirmed == nil then
		local currentSystem = celestial.currentSystem()
		local destination = { location = travel or self.travel.system, planet = 0, satellite = 0, system = self.travel.target }
		local distanceCalc = math.sqrt( ( (currentSystem.location[1] - destination.location[1]) ^ 2 ) + ( (currentSystem.location[2] - destination.location[2]) ^ 2 ) )

        showJumpDialog(self.travel.system, self.travel.target, distanceCalc)
      end
    end
        ...
        if button == 2 then
          if canFlyShip(system) then
			...
			if to[1] == "coordinate" and celestial.planetParameters(to[2]).worldType ~= "FloatingDungeon" then
				...
				travel = { system = system.location, target = to }
			end
			local shipLocation = celestial.shipLocation()
			local destination = nil
			if travel.target[1] == "orbit" then
				destination = travel.target[2].target.planet
			elseif travel.target[1] == "object" then
				destination = travel.target[2]
			end
			if self.travel.confirmed == nil and destination ~= nil and compare(system, celestial.currentSystem()) then
				if shipLocation[1] == "orbit" and compare(shipLocation[2].target.planet, destination) then
					self.travel = travel
				elseif shipLocation[1] == "orbit" and compare(shipLocation[2].target.planet, destination) == false then
					showJumpDialogInSystem(travel)
				elseif shipLocation[1] == "coordinate" and compare(shipLocation[2].planet, destination) == false then
					showJumpDialogInSystem(travel)
				elseif shipLocation[1] == "coordinate" and compare(shipLocation[2].planet, destination) == true then
					self.travel = travel
				elseif shipLocation[1] == "object" and compare(shipLocation[2], destination) == false then
					showJumpDialogInSystem(travel)
				elseif shipLocation[1] == "object" and compare(shipLocation[2], destination) == true then
					self.travel = travel
				else
					self.travel = travel
				end
      ...
      if down then
        ...
        if button == 2 and hover then
          if canFlyShip(system) and locationVisitable(hover) then
            pane.playSound(self.sounds.startTravel)
			local shipLocation = celestial.shipLocation()
			local travel = {system = system.location, target = hover}
			if self.travel.confirmed == nil and compare(system, celestial.currentSystem()) == true then
				if shipLocation[1] == "orbit" and compare(shipLocation[2].target.planet, travel.target[2].planet) then
					self.travel = {system = system.location, target = hover}
				elseif shipLocation[1] == "coordinate" and compare(shipLocation[2].planet, travel.target[2].planet) == false then
					showJumpDialogInSystem(travel)
				elseif shipLocation[1] == "coordinate" and compare(shipLocation[2].planet, travel.target[2].planet) == true then
					self.travel = {system = system.location, target = hover}
				end
			elseif self.travel.confirmed == nil and compare(system, celestial.currentSystem()) == false then
				pane.playSound(self.sounds.failTravel)
			else
				self.travel = {system = system.location, target = hover}
			end
