...
function update(dt)
	...
	local worldeffectslist = world.environmentStatusEffects(worldMouthPosition)
	local worldefftable = sb.printJson(worldeffectslist)
	...
	if effect.duration() and string.find(worldefftable, "biomecold") ~= nil and world.windLevel(worldMouthPosition) > 0 then
		effect.modifyDuration(dt)
	end
end
